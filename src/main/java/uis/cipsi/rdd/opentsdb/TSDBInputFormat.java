...
public class TSDBInputFormat extends TableInputFormatBase implements
		Configurable {
			...
			try {
				...
				if (conf.get(TSDB_UIDS) != null) {
					...
					if (conf.get(METRICS) != null) {
						...
						RegexStringComparator keyRegEx = new RegexStringComparator(
								filter, Pattern.CASE_INSENSITIVE
										| Pattern.DOTALL);
					...
					if (conf.get(TSDB_TIMERANGE_START) != null
							&& conf.get(TSDB_TIMERANGE_END) != null) {
...
//						System.out.println(hexStringToByteArray(metrics + conf.get(TSDB_TIMERANGE_END) + tags));
	...
	public List<InputSplit> getSplits(JobContext context) throws IOException {
		...
		if ((conf.get(SHUFFLE_MAPS) != null)
				&& "true".equals(conf.get(SHUFFLE_MAPS).toLowerCase())) {
